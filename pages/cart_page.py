...
class CartPage(BasePage):
    ...
    total_amount_value = (By.XPATH, "//*[@id='totals_table']/tbody/tr[3]/td[2]/span")
    totals_table = (By.ID, "totals_table")
    flat_shipping_rate = (By.XPATH, "//*[@id='totals_table']/tbody/tr[2]/td[2]/span") 
    ...
    def cheapest_product_on_cart(self) -> WebElement:
        ...
        cheapest_price = float(self.get_product_unit_price_in_cart(products[0]))
        ...
        py_logger.info(f"Найден самый дешёвый товар {self.get_product_name_in_cart(cheapest_product)} (${cheapest_price})")
    ...
    def change_product_quantity(self, quantity, product):
        ...
        py_logger.info(f"Успешно изменено количество товара {self.get_product_name_in_cart(product)} на {quantity}")
        return self
    ...
    def calculating_total_price(self) -> float:
        """Вычисляет общую сумму корзины на основе количества и цены каждого товара"""
        products = self.get_all_products_in_cart()
        total = 0.0
    ...
    def remove_even_products_from_cart(self) -> None:
        """Удаляет товары, находящиеся на четных позициях в корзине (начиная с 1)""" 

        products = self.get_all_products_in_cart()
        py_logger.info(f"Сейчас в корзине товары: {self.get_all_products_names_in_cart()}")
    ...
    def increase_cheapest_product_quantity(self) -> 'CartPage':
        ...
        py_logger.info(f"Текущее количество товара {self.get_product_name_in_cart(cheapest_product)}")
