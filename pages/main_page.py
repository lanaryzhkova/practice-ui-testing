...
class MainPage(BasePage):
    ...
    option_name_az = (By.CSS_SELECTOR,
                      "#sort > option[value='pd.name-ASC']")
    option_name_za = (By.CSS_SELECTOR,
                      "#sort > option[value='pd.name-DESC']")
    option_price_low_high = (By.CSS_SELECTOR,
                             "#sort > option[value='p.price-ASC']")
    option_price_high_low = (By.CSS_SELECTOR,
                             "#sort > option[value='p.price-DESC']")
    grid_products_in_category = (By.CSS_SELECTOR,
                                 "#maincontainer "
                                 ".thumbnails.grid.row.list-inline")
    grid_products_in_home = (By.CSS_SELECTOR,
                             "#maincontainer .thumbnails.list-inline")
    ...
    def click_category(self, category_name: str) -> 'MainPage':
        ...
        for category in categories:
            if category_name in category.text:
                self.click(category)
                py_logger.info(f"Выбрана категория {category_name}")
                return self
        py_logger.warning(f"Категория '{category_name}' не найдена")
        raise ValueError(f"Категория '{category_name}' не найдена")

    @allure.step("Выбор товара по позиции {position} в категории")
    def click_product_by_position_in_category(self,
                                              position: int) -> 'MainPage':
        """Кликает на товар по его позиции на странице категории"""
        ...
        if position < 1 or position > len(products):
            raise ValueError(
                f"Позиция {position} вне диапазона доступных товаров")
        self.click(products[position - 1])
        return self

    def click_product_by_position_in_home(self, position: int) -> 'MainPage':
        """Кликает на товар по его позиции на главной странице"""
        products = [p for p in self.find_elements(self.name_product)
                    if p.is_displayed()]
    ...
    def select_sorting_option(self, option_text: str) -> 'MainPage':
        ...
        self.click(self.find_element(self.sorting_select))
        options = [
            self.find_element(self.option_name_az),
            self.find_element(self.option_name_za),
            self.find_element(self.option_price_low_high),
            self.find_element(self.option_price_high_low)
        ]
        for option in options:
            if self.text_of(option) == option_text:
                self.click(option)
                return self
        raise ValueError(f"Опция сортировки '{option_text}' не найдена")

    def get_product_names_in_category(self) -> list[str]:
        """Возвращает список названий товаров на странице категории"""
        grid_products = self.find_element(self.grid_products_in_category)
        products = [p for p in grid_products.find_elements(*self.name_product)
                    if p.is_displayed()]
        names = []
        for product in products:
            names.append(self.text_of(product))
        return names

    def get_product_names_in_home(self) -> list[str]:
        """Возвращает список названий товаров на главной странице"""
    ...
    def get_product_prices_in_category(self) -> list[float]:
        ...
        prices = []
        for product in products:
            prices.append(self.text_of(product))
